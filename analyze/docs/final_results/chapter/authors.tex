...
Durch die im Abschnitt \ref{communities} gefundenen Communities kann man nun auch die Beziehung 
...
Falls verschiedene Rollenverteilungen auftreten, soll festgestellt werden, welche Rollen die Autoren einnehmen. Zusätzlich könnten die Rollen der Autoren, welche am 
meisten publizieren, betrachtet werden. \\

% { Methoden }

Um die Autoren mit verschiedenen Rollen charakterisieren zu können, werden die in \cite{hubsconnectors} beschriebene Klassifizierung von Knoten verwendet. Für diese Klassifizierung muss für jeden Knoten der \textit{Within-module-degree} (z-score) und der \textit{Participation coefficient} berechnet werden. 
...
In diesem Modell wird zwischen folgenden Rollen unterschieden (Es wurde versucht, eine möglichst passende deutsche Übersetzung zu finden ):
...
Der Within-module degree eines Autors ist durch folgende Formel \cite{hubsconnectors} bestimmt: \\
...
Der Participation-coefficient eines Autors ist durch folgende Formel \cite{hubsconnectors} bestimmt: \\
...
In Abbildung \ref{fig:hub_con_dist} ist die Verteilung zwischen \textit{Hubs} und \textit{Connectors} zu erkennen. 
...
Abbildung \ref{fig:r1_r7_dist} zeigt die Verteilung auf alle Rollen. Es wurden zu jeder definierten Rolle Autoren gefunden. Dabei gibt es am meisten \textit{R1 Autoren} und am wenigsten \textit{R7 Autoren}.
...
Folgende Tabelle zeigt verschiedene Details der meist publizierenden Autoren (Top 16 der DBLP - Stand 26.01.2012). Dabei beziehen sich die Conductance und die Größe auf die Community in der sich der Autor befindet.

\begin{table}[htb!]
    \begin{tabular}{|l|l|l|l|l|}
        \hline
        \textbf{Name }    & \textbf{Anzahl Publikationen} &\textbf{ Rolle} & \textbf{Conductance}   & \textbf{Größe }    \\ \hline 
        ...
        \hline
    \end{tabular}
    \caption{Meist publizierende Autoren}
\label{tab:mostProlific}
\end{table}
